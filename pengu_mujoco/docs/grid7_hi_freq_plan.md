...
## 补扫完成后再做（Ben 2026-09-11）
- c1 μ0.1：现在的正走格子（第 79 名 1.5/220/110/32/40）右脚全程拖地、只有左脚迈步，要找一个左右**均衡**的（两脚交替、DF 接近）。补扫后在新排名里按"|DF_L − DF_R| 小 + 朝向 ≤ 60°"找。
- c6 μ0.1：1.83/120/75/32/20 的代表周期在 36–38 % 和 94 % 处各有一个小零承重缝，要么换一个没有缝的格子（补扫后在 1.75–2.00 Hz 里找零承重 0 % 的），要么接受。
- 两条都等 `_hi` 数据回来再做，不单独跑。

// pengu_hw_c4.ino — pengu_tune_wifi with the c4 cap-only-sweep champions as presets 1 / 2, PID torso.
//
...
//
// c4: kappa = 2, COM ratio 1.05  (sim model pengu1_05_hw_updated). SET THE BALLAST FOR 1.05 BEFORE WALKING.
...
//   1   1.65 / 290 / 95 / 32 / 25   mu 0.12 (ice)   sim v 0.366 m/s, clear 10 mm, torso rms 24.2, clamp 0% of the time
//   2   1.65 / 260 / 130 / 32 / 40   mu 0.45 (floor) sim v 0.466 m/s, clear 37 mm, torso rms 31.4, clamp 0% of the time
//   3   1.39 / 240 /  80 / 16 / 30  the torso calibration gait (kappa 0, feedforward)
//   runners-up, mu 0.12:
//        1.65 / 280 / 90 / 32 / 20   v 0.358  clear 13 mm  torso rms 23.0  clamp 0%
//        1.60 / 280 / 95 / 32 / 20   v 0.345  clear 11 mm  torso rms 23.5  clamp 0%
//   runners-up, mu 0.45:
//        1.65 / 270 / 130 / 32 / 40   v 0.465  clear 36 mm  torso rms 32.3  clamp 0%
//        1.65 / 270 / 125 / 32 / 40   v 0.460  clear 35 mm  torso rms 32.2  clamp 0%
...
// J* = (kappa-1)*axis, slower convergence. (b) the clamp: model 45 deg, robot 25 (stop at +27.8/-31).
...
float p_hipPhi  = 290.0f;     // [deg] hip phase relative to the crank
...
void apply_cmd(char cmd) {
  switch (cmd) {
    ...
    case '1':   // c4 cap-only champion, mu 0.12 ice: 1.65 / 290 / 95 / 32 / 25, sim 0.366 m/s, 10 mm worst-cycle clearance,
                // torso roll 24.2 deg rms, nbhd pass 1.00, crank demand 492 deg/s (cap 354, modelled).
      setFreq(1.65f); p_legAmp = 95.0f; p_hipAmp = 32.0f; p_hipPhi = 290.0f; p_hipOff = 25.0f;
      torso_mode = TM_PID;
      announce(); break;
    case '2':   // c4 cap-only champion, mu 0.45 floor: 1.65 / 260 / 130 / 32 / 40, sim 0.466 m/s, 37 mm worst-cycle clearance,
                // torso roll 31.4 deg rms, nbhd pass 1.00, crank demand 674 deg/s (cap 354, modelled).
      setFreq(1.65f); p_legAmp = 130.0f; p_hipAmp = 32.0f; p_hipPhi = 260.0f; p_hipOff = 40.0f;
      torso_mode = TM_PID;
      announce(); break;

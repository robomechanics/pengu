// pengu_hw_c1.ino — pengu_tune_wifi with the c1 hardware-sweep champions as presets 1 / 2.
//
...
//
// c1: kappa = 0, COM ratio 1.05  (sim model pengu1_05_hw_updated). SET THE BALLAST FOR 1.05 BEFORE WALKING.
...
//   1   1.55 / 260 / 125 / 28 / 25  ff 17.8 / 165   mu 0.12 (ice)   sim v 0.276 m/s, clear 11 mm, torso rms 22.5, clamp 0% of the time
//   2   1.65 / 30 / 80 / 32 / 40  ff 37.8 / 140   mu 0.45 (floor) sim v 0.594 m/s, clear 29 mm, torso rms 41.7, clamp 54% of the time
//   3   1.39 / 240 /  80 / 16 / 30  the torso calibration gait (kappa 0)
//   runners-up, mu 0.12:
//        1.60 / 330 / 130 / 32 / 20  ff 21.3 / 313   v 0.274  clear 12 mm  torso rms 29.1
//        1.60 / 340 / 125 / 32 / 40  ff 31.0 / 320   v 0.273  clear 12 mm  torso rms 37.5
//   runners-up, mu 0.45:
//        1.65 / 20 / 80 / 32 / 40  ff 38.4 / 132   v 0.546  clear 22 mm  torso rms 41.3
//        1.60 / 20 / 80 / 32 / 40  ff 39.2 / 11   v 0.532  clear 19 mm  torso rms 37.6
// ff_phi is the model's (axis-roll phase + best lead + 180); it is a starting point -- scan it with h/H.
// KAPPA = 0: the torso cancels the axis roll (ff_phi carries the 180).
//
...
float p_legFreq = 1.55f;      // [Hz]  the executable champion, to the 0.05 step grid
...
float         ff_amp  = 17.8f;    // [deg]  measured on the robot, see above
float         ff_phi  = 165.4f;  // [deg] relative to the gait phase. The naive value that
...
void apply_cmd(char cmd) {
  switch (cmd) {
    ...
    case '1':   // c1 champion, mu 0.12 ice: 1.55 / 260 / 125 / 28 / 25, sim 0.276 m/s, 11 mm worst-cycle clearance,
                // torso roll 22.5 deg rms, nbhd pass 0.89, crank demand 609 deg/s (cap 354, modelled).
      setFreq(1.55f); p_legAmp = 125.0f; p_hipAmp = 28.0f; p_hipPhi = 260.0f; p_hipOff = 25.0f;
      ff_amp = 17.8f; ff_phi = 165.4f; ff_gain = 1.0f;
      announce(); break;
    case '2':   // c1 champion, mu 0.45 floor: 1.65 / 30 / 80 / 32 / 40, sim 0.594 m/s, 29 mm worst-cycle clearance,
                // torso roll 41.7 deg rms, nbhd pass 1.00, crank demand 415 deg/s (cap 354, modelled).
      setFreq(1.65f); p_legAmp = 80.0f; p_hipAmp = 32.0f; p_hipPhi = 30.0f; p_hipOff = 40.0f;
      ff_amp = 37.8f; ff_phi = 139.5f; ff_gain = 1.0f;
      announce(); break;

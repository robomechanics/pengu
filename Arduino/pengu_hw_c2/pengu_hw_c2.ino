// pengu_hw_c2.ino — pengu_tune_wifi with the c2 hardware-sweep champions as presets 1 / 2.
//
...
//
// c2: kappa = 0, COM ratio 1.20  (sim model pengu1_20_hw_updated). SET THE BALLAST FOR 1.20 BEFORE WALKING.
...
//   1   1.50 / 300 / 100 / 32 / 20  ff 23.0 / 78   mu 0.12 (ice)   sim v 0.486 m/s, clear 66 mm, torso rms 20.2, clamp 0% of the time
//   2   1.65 / 270 / 100 / 24 / 30  ff 19.3 / 50   mu 0.45 (floor) sim v 0.449 m/s, clear 20 mm, torso rms 22.1, clamp 0% of the time
//   3   1.39 / 240 /  80 / 16 / 30  the torso calibration gait (kappa 0)
//   runners-up, mu 0.12:
//        1.55 / 270 / 125 / 28 / 25  ff 23.4 / 185   v 0.467  clear 25 mm  torso rms 26.7
//        1.55 / 270 / 130 / 28 / 25  ff 23.9 / 184   v 0.466  clear 25 mm  torso rms 26.6
//   runners-up, mu 0.45:
//        1.65 / 280 / 100 / 20 / 35  ff 20.0 / 46   v 0.444  clear 34 mm  torso rms 23.1
//        1.65 / 300 / 70 / 24 / 35  ff 18.2 / 45   v 0.442  clear 23 mm  torso rms 26.0
...
float p_legFreq = 1.50f;      // [Hz]  the executable champion, to the 0.05 step grid
float p_legAmp  = 0.0f;       // [deg] crank amplitude
float p_hipAmp  = 0.0f;       // [deg] hip swing amplitude
float p_hipPhi  = 300.0f;     // [deg] hip phase relative to the crank
...
float         ff_amp  = 23.0f;    // [deg]  measured on the robot, see above
float         ff_phi  = 77.5f;  // [deg] relative to the gait phase. The naive value that
...
void apply_cmd(char cmd) {
  switch (cmd) {
    ...
    case '1':   // c2 champion, mu 0.12 ice: 1.50 / 300 / 100 / 32 / 20, sim 0.486 m/s, 66 mm worst-cycle clearance,
                // torso roll 20.2 deg rms, nbhd pass 0.89, crank demand 471 deg/s (cap 354, modelled).
      setFreq(1.50f); p_legAmp = 100.0f; p_hipAmp = 32.0f; p_hipPhi = 300.0f; p_hipOff = 20.0f;
      ff_amp = 23.0f; ff_phi = 77.5f; ff_gain = 1.0f;
      announce(); break;
    case '2':   // c2 champion, mu 0.45 floor: 1.65 / 270 / 100 / 24 / 30, sim 0.449 m/s, 20 mm worst-cycle clearance,
                // torso roll 22.1 deg rms, nbhd pass 0.89, crank demand 518 deg/s (cap 354, modelled).
      setFreq(1.65f); p_legAmp = 100.0f; p_hipAmp = 24.0f; p_hipPhi = 270.0f; p_hipOff = 30.0f;
      ff_amp = 19.3f; ff_phi = 50.5f; ff_gain = 1.0f;
      announce(); break;

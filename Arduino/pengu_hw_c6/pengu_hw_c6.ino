// pengu_hw_c6.ino — pengu_tune_wifi with the c6 hardware-sweep champions as presets 1 / 2.
//
...
//
// c6: kappa = 2, COM ratio 1.31  (sim model pengu1_31). SET THE BALLAST FOR 1.31 BEFORE WALKING.
// Presets 1 and 2 are GRID-6 hardware-model sweep cells (2026-09-09): 354 deg/s crank cap, 56 ms torso lag,
// feedforward torso, CAD model; taken from the top-40 robust (nbhd >= 0.8) + clearance (>= 10 mm) cells and
// re-simulated with five feedforward leads: the flashed cell is the fastest whose EVERY lead walks (Ben:
// a single-lead survivor is not a champion). See results/grid6_hw/demos/lead_window_c6_mu0*.csv.
//   1   1.60 / 260 / 115 / 24 / 20  ff 18.6 / 209   mu 0.12 (ice)   sim 0.19-0.44 m/s over leads 10-90 (all walk); PSC 0.390
//   2   1.60 / 260 / 125 / 16 / 35  ff 8.3 / 75    mu 0.45 (floor) sim 0.19-0.25 m/s over leads 10-90 (all walk); PSC 0.213
//   3   1.39 / 240 /  80 / 16 / 30  the torso calibration gait (kappa 0)
//   runners-up, mu 0.12:
//        1.35 / 60 / 115 / 24 / 20  ff 20.2 / 52   v 0.431  clear 16 mm  torso rms 27.8
//        1.35 / 60 / 130 / 24 / 25  ff 23.3 / 45   v 0.424  clear 27 mm  torso rms 33.5
//   runners-up, mu 0.45:
//        1.65 / 260 / 115 / 20 / 35  ff 8.2 / 200   v 0.243  clear 20 mm  torso rms 13.7
//        1.60 / 260 / 125 / 16 / 35  ff 8.2 / 103   v 0.213  clear 14 mm  torso rms 12.3
// ff_phi is the model's (axis-roll phase + best lead); it is a starting point -- scan it with h/H.
// KAPPA = 2: the torso LEANS WITH the axis (ff_phi carries no 180). The sim clamped the torso at 45 deg
// for kappa 2; this robot's stop is 25, so any command past 25 is flattened here (see clamp % above).
//
...
const float    KAPPA           = 2.0f;   // torso target = kappa * hip-axis roll. 0 = hold level
...
float p_legFreq = 1.60f;      // [Hz]  the executable champion, to the 0.05 step grid
float p_legAmp  = 0.0f;       // [deg] crank amplitude
float p_hipAmp  = 0.0f;       // [deg] hip swing amplitude
float p_hipPhi  = 260.0f;     // [deg] hip phase relative to the crank
...
float         ff_amp  = 18.6f;    // [deg]  measured on the robot, see above
float         ff_phi  = 209.0f;  // [deg] relative to the gait phase. The naive value that
...
void apply_cmd(char cmd) {
  switch (cmd) {
    ...
    case '1':   // c6, mu 0.12 ice: 1.60 / 260 / 115 / 24 / 20 (Ben 2026-09-09: "give me the 1.60 one"). Re-simulated with
                // cap 354 + 56 ms + FF: every lead 10/30/50/70/90 walks, 0.194 / 0.370 / 0.436 / 0.305 / 0.389 m/s; PSC row
                // 0.390, nbhd pass >= 0.8, clearance ok. ff_phi 209 = axis phase 159 + lead 50 (the fastest lead, with
                // lead 30 / 70 on either side at 0.37 / 0.31). Crank demand 578.
                // Alternatives from the same scan: 1.65/280/90/28/20 (all leads 0.35-0.51, widest margin), 1.60/250/130/24/20
                // (all leads walk, 0.19-0.43), 1.60/250/125/24/20 (all leads walk, 0.19-0.41).
      setFreq(1.60f); p_legAmp = 115.0f; p_hipAmp = 24.0f; p_hipPhi = 260.0f; p_hipOff = 20.0f;
      ff_amp = 18.6f; ff_phi = 209.0f; ff_gain = 1.0f;
      announce(); break;
    case '2':   // c6 champion, mu 0.45 floor: 1.60 / 260 / 125 / 16 / 35 -- same rule. Every lead 10..90 walks,
                // 0.188-0.250 m/s (mean 0.222); PSC row 0.213, nbhd ok, clearance ok. ff_phi 75 = axis phase 35 + lead 40
                // (leads 30 and 50 read 0.250 / 0.229); crank demand 628.
      setFreq(1.60f); p_legAmp = 125.0f; p_hipAmp = 16.0f; p_hipPhi = 260.0f; p_hipOff = 35.0f;
      ff_amp = 8.3f; ff_phi = 75.0f; ff_gain = 1.0f;
      announce(); break;

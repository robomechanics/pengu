// pengu_hw_c5.ino — pengu_tune_wifi with the c5 hardware-sweep champions as presets 1 / 2.
//
...
//
// c5: kappa = 2, COM ratio 1.20  (sim model pengu1_20_hw_updated). SET THE BALLAST FOR 1.20 BEFORE WALKING.
// Presets 1 and 2 are the GRID-6 hardware-model sweep champions (2026-09-09): 354 deg/s crank
// cap, 56 ms torso lag, feedforward torso, CAD model; selected among cells that walk with
// every freq +-0.05 / hip_phi +-10 neighbour (nbhd pass >= 0.8) AND clear the floor by
// >= 10 mm at the worst cycle, then ranked on whole-body-COM net speed.
//   1   1.65 / 230 / 130 / 32 / 25  ff 24.2 / 242   mu 0.12 (ice)   sim v 0.433 m/s, clear 34 mm, torso rms 16.2, clamp 0% of the time
//   2   1.50 / 290 / 90 / 24 / 25  ff 17.6 / 218   mu 0.45 (floor) sim v 0.405 m/s, clear 20 mm, torso rms 20.0, clamp 0% of the time
//   3   1.39 / 240 /  80 / 16 / 30  the torso calibration gait (kappa 0)
//   runners-up, mu 0.12:
//        1.65 / 260 / 120 / 28 / 35  ff 29.8 / 262   v 0.426  clear 36 mm  torso rms 25.6
//        1.65 / 230 / 125 / 32 / 25  ff 22.7 / 242   v 0.401  clear 31 mm  torso rms 15.4
//   runners-up, mu 0.45:
//        1.60 / 160 / 85 / 32 / 25  ff 11.8 / 175   v 0.376  clear 25 mm  torso rms 18.8
//        1.45 / 240 / 130 / 24 / 20  ff 17.0 / 96   v 0.376  clear 45 mm  torso rms 15.5
...
float p_legFreq = 1.65f;      // [Hz]  the executable champion, to the 0.05 step grid
float p_legAmp  = 0.0f;       // [deg] crank amplitude
float p_hipAmp  = 0.0f;       // [deg] hip swing amplitude
float p_hipPhi  = 230.0f;     // [deg] hip phase relative to the crank
float p_hipOff  = 25.0f;      // [deg] forward lean
...
float         ff_amp  = 24.2f;    // [deg]  measured on the robot, see above
float         ff_phi  = 241.6f;  // [deg] relative to the gait phase. The naive value that
...
void apply_cmd(char cmd) {
  switch (cmd) {
    ...
    case '1':   // c5 champion, mu 0.12 ice: 1.65 / 230 / 130 / 32 / 25, sim 0.433 m/s, 34 mm worst-cycle clearance,
                // torso roll 16.2 deg rms, nbhd pass 1.00, crank demand 674 deg/s (cap 354, modelled).
      setFreq(1.65f); p_legAmp = 130.0f; p_hipAmp = 32.0f; p_hipPhi = 230.0f; p_hipOff = 25.0f;
      ff_amp = 24.2f; ff_phi = 241.6f; ff_gain = 1.0f;
      announce(); break;
    case '2':   // c5 champion, mu 0.45 floor: 1.50 / 290 / 90 / 24 / 25, sim 0.405 m/s, 20 mm worst-cycle clearance,
                // torso roll 20.0 deg rms, nbhd pass 1.00, crank demand 424 deg/s (cap 354, modelled).
      setFreq(1.50f); p_legAmp = 90.0f; p_hipAmp = 24.0f; p_hipPhi = 290.0f; p_hipOff = 25.0f;
      ff_amp = 17.6f; ff_phi = 217.9f; ff_gain = 1.0f;
      announce(); break;
